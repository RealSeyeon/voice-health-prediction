...
for gender_value, gender_name in [(0, 'male'), (1, 'female')]:
    ...
    writer = SummaryWriter(log_dir=f'runs/transformer_transfer_{gender_name}')
    ...
    for epoch in range(50):
        ...
        if accuracy > best_acc or (accuracy == best_acc and val_loss < best_loss):
            ...
            counter = 0
            torch.save(classifier.state_dict(), f'transformer_transfer_{gender_name}.pth')
        else:
            counter += 1
            if counter >= patience:
    ...
    writer.close()
    print(f'Completed transformer transfer learning for {gender_name}. Best Accuracy: {best_acc:.4f}')
